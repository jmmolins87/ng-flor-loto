


import { cardsInterface } from "../models/cards/cards.interface";


export const funerals: cardsInterface[] = [
  {
    srcImg: ["/assets/img/products/funerals/funeral-centro_funerario_azul_y_naranja.jpeg"],
    titleCard: "FUNERALS_PAGE.BLUE_AND_ORANGE_FUNERAL_CENTER.TITLE",
    descriptionCard: "FUNERALS_PAGE.BLUE_AND_ORANGE_FUNERAL_CENTER.DESCRIPTION"
  },
  {
    srcImg: ["/assets/img/products/funerals/funeral-centro_funerario_bidasoa.jpeg"],
    titleCard: "FUNERALS_PAGE.BIDASOA_FUNERAL_CENTER.TITLE",
    descriptionCard: "FUNERALS_PAGE.BIDASOA_FUNERAL_CENTER.DESCRIPTION"
  },
  {
    srcImg: ["/assets/img/products/funerals/funeral-centro_funerario_semblante.jpeg"],
    titleCard: "FUNERALS_PAGE.FUNERAL_CENTER_APPEARANCE.TITLE",
    descriptionCard: "FUNERALS_PAGE.FUNERAL_CENTER_APPEARANCE.DESCRIPTION"
  },
  {
    srcImg: ["/assets/img/products/funerals/funeral-centro_funerario-F12.jpeg"],
    titleCard: "FUNERALS_PAGE.F12_FUNERAL_CENTER.TITLE"
  },
  {
    srcImg: ["/assets/img/products/funerals/ramo_funerario.jpeg"],
    titleCard: "FUNERALS_PAGE.FUNERAL_BOUQUET.TITLE"
  },
  {
    srcImg: ["/assets/img/products/funerals/funeral-centro_funerario_espana.jpeg"],
    titleCard: "FUNERALS_PAGE.SPAIN_FUNERAL_CENTER.TITLE"
  },
  {
    srcImg: ["/assets/img/products/funerals/funeral-centro-F123.jpeg"],
    titleCard: "FUNERALS_PAGE.FUNERAL_CENTER_F123.TITLE"
  },
  {
    srcImg: ["/assets/img/products/funerals/funeral-centro-F111.jpeg"],
    titleCard: "FUNERALS_PAGE.LAUREL_CENTER_F111.TITLE"
  },
  {
    srcImg: ["/assets/img/products/funerals/funeral-corona_lisboa.jpeg"],
    titleCard: "FUNERALS_PAGE.LAUREL_CENTER_LISBOA.TITLE",
    descriptionCard: "FUNERALS_PAGE.LAUREL_CENTER_LISBOA.DESCRIPTION"
  }
]
